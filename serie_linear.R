########## melhor resultado
load(file="final.rda")
final <- rbind(final2002,final2008,final2009,final2012,final2015,final2016,final2018)
...
b <- c()
for(i in 1:length(a)){
  wr <- (a[1:i]>0) == (teste19$result[1:i] > 0)
  if(!is.na(a[i])){
    b[i] <- mean(wr, na.rm=T)
  }else{
    b[i] <- NaN
